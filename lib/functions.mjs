...
// Credit: https://stackoverflow.com/questions/175739/how-can-i-check-if-a-string-is-a-valid-number
//
...
let getAllRows = async function (tablename, orderby = "''") {
    ...
    const sqlquery = `SELECT * FROM ${tablename} ORDER BY ${orderby};`;
    let allrows;
    try {
        allrows = await queryDB(sqlquery, [tablename, orderby]);
    } catch (err) {
...
let displayTable = async function (tablename, orderby = "") {
    let headers = (await getColumnNames(tablename)).map((element) => element.COLUMN_NAME);
    // console.log("displayTable headers:", headers); debug log
    if (orderby === "" && headers.length > 0) {
        orderby = headers[0];
    };
    let rows = await getAllRows(tablename, orderby);
...
export { getColumnNames, getAllRows, displayTable, displaySQLQueryTable, getSQLQuery, isNumeric };
